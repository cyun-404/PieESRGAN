...
def train_generator(train_dataloader, epoch) -> None:
    ...
    for index, (lr, hr) in enumerate(train_dataloader):
        ...
        if (index + 1) % 10 == 0 or (index + 1) == batches:
            print(f"Train Epoch[{epoch + 1:04d}/{p_epochs:04d}]np.{index + 1:05d}/{batches:05d}) "
...
def train_adversarial(train_dataloader, epoch) -> None:
    ...
    for index, (lr, hr) in enumerate(train_dataloader):
        ...
        g_loss = pie.mean().item() + adversarial_loss + content_loss + pixel_loss  #0.4* pieapp_loss.item() ++ content_loss  pie.mean().item() + 
